...
- `path_lazy` — one shared source too large to reproject/cache whole (e.g. a mosaic VRT over scattered regional tiles whose combined bounding box is much bigger than the actual data coverage). Cropped to each site's catchment *before* being reprojected or written to disk — never materializes the full source extent. Also resolved lazily per site.
