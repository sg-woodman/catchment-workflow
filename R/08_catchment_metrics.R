...
#
# Outputs:
#   metrics tibble : Two rows per site (unclipped/clipped), one column per
#                    metric plus a "version" column
...
#'
#' Iterates over all sites, computing metrics for both the unclipped
#' catchment/DEM (from 05_delineate_sites.R) and the clipped catchment/DEM
#' (from 07_reclip_outputs.R). Each site contributes up to two rows, tagged
#' by the "version" column. Sites missing the unclipped catchment.gpkg are
#' skipped entirely with a warning; sites missing the clipped
#' catchment_clipped.gpkg produce only the unclipped row, with a warning.
#'
...
#'
#' Loads the specified catchment and DEM files and computes geometry, areal,
#' and relief metrics. Returns NULL if the catchment file does not exist.
#' If the DEM file does not exist, relief metrics are returned as NA but
#' geometry/areal metrics are still computed.
#'
#' @param site_dir       Character. Site output directory path
#' @param site_id        Character. Site identifier (for log messages)
#' @param version        Character. "unclipped" or "clipped" — written to
#'                        the output as a column for downstream filtering
#' @param catchment_file Character. Filename of the catchment polygon to use
#'                        (e.g. "catchment.gpkg" or "catchment_clipped.gpkg")
#' @param dem_file       Character. Filename of the DEM to use
#'                        (e.g. "dem.tif" or "dem_clipped.tif")
#'
...
compute_site_metrics <- function(
  ...
  relief <- compute_relief_metrics(dem, geom, site_id)
...
#'
#' @param metrics    Tibble from calculate_catchment_metrics() — contains a
#'                    "version" column distinguishing "unclipped"/"clipped" rows
...
# Source the script (or run it if using targets/source())
source(here("R/utils.R"))

# Compute metrics for all sites — produces two rows per site
# (version = "unclipped" and "clipped") where available
metrics <- calculate_catchment_metrics(
  cam_sites,
  "/Users/sam/Documents/cfs/catchment-workflow/output/CAM"
)

# Build the reference table explaining each metric
ref_table <- build_metrics_reference_table()

# Write both outputs to disk
write_metrics_outputs(metrics, ref_table, output_dir)
