...
# other project in this repo now. CAM lakes' own workflow/run_cam_lakes.R
# (workflow/R/lake/ pipeline) is DELIBERATELY left untouched and still the
# production runner — this is a new, parallel run for verification first,
# same pattern CELESTE went through (workflow/CELESTE/run_celeste.R vs. the
# retired non-modular pipeline) before being trusted as a replacement.
#
# output/CAM/lake_delineation_engine/ and cache/CAM_lakes_engine/ are
# separate from production (output/CAM/lake_delineation/, cache/CAM/), which
# this script never reads from or writes to.
#
...
# delineate_engine_lake_site(), adapted line-for-line from
# lake/03_delineate_lakes.R's delineate_single_lake() when the engine was
# first built) but had never actually been run. Terrain prep is also already
# solved — workflow/CAM/run_cam_streams.R already runs OIH's Enhanced Flow
# Direction through engine/02_prepare_terrain.R's recode+accumulate+extract-
# streams path, identical physical data this project uses. So this runner is
# mostly wiring together already-built, already-proven pieces:
#
...
#       engine's terrain prep writes flow_pointer.tif instead. Fixed below
#       with a cheap symlink alias right after Stage 2 — same pattern
#       engine/02_prepare_terrain.R's own alias_dem_breached() already
#       establishes for an analogous naming gap — rather than modifying the
#       shared function itself (used only by this one pipeline, but touching
#       it risks the still-active production run_cam_lakes.R too).
#
# WORKFLOW STAGES:
